 ...
 * @author Anshu_Virk
 */
public class RaidDelPendConfigCmd extends AbstractRaid {

    private List<KeyValuePair> keyValue = new ArrayList<KeyValuePair>();
    private static final Logger logger = LoggerFactory.getLogger(RaidDelPendConfigCmd.class);


    public RaidDelPendConfigCmd(String ipAddr, String userName, String passwd) {
        super(ipAddr, userName, passwd);
        if (logger.isTraceEnabled()) {
            logger.trace(String.format("Entering constructor: RaidDelPendConfigCmd(String ipAddr - %s, String userName - %s, String passwd - %s)", ipAddr, userName, "####"));
        }
        logger.trace("Exiting constructor: RaidDelPendConfigCmd()");
    }
    ...
    public Object execute() throws Exception {
        ...
        this.addSelectors();
        this.addUserParms();
        session.setInvokeCommand(RaidWSManMethodEnum.DeletePendingConfiguration.getDisplayString());
