 ...
 * @author Anshu_Virk
 */
public class RaidAssignSpareCmd extends AbstractRaid {

    private List<KeyValuePair> keyValue = new ArrayList<KeyValuePair>();
    private static final Logger logger = LoggerFactory.getLogger(RaidAssignSpareCmd.class);


    public RaidAssignSpareCmd(String ipAddr, String userName, String passwd) {
        super(ipAddr, userName, passwd);
        if (logger.isTraceEnabled()) {
            logger.trace(String.format("Entering constructor: RaidAssignSpareCmd(String ipAddr - %s, String userName - %s, String passwd - %s)", ipAddr, userName, "####"));
        }
        logger.trace("Exiting constructor: RaidAssignSpareCmd()");
    }
    ...
    public Object execute() throws Exception {
        ...
        session.setInvokeCommand(RaidWSManMethodEnum.AssignSpare.getDisplayString());
    ...
    public void setKeyValues(List<KeyValuePair> keyValues) {
        ...
        for (KeyValuePair keyValuePair : keyValues) {
            if (keyValuePair.getKey().equalsIgnoreCase(RaidWSManMethParmEnum.Target.getDisplayString())) {
                ...
                keyValue.add(keyVal);
            } else if (keyValuePair.getKey().equalsIgnoreCase(RaidWSManMethParmEnum.VirtualDiskArray.getDisplayString())) {
                keyVal = new KeyValuePair();
                keyVal.setKey(RaidWSManMethParmEnum.VirtualDiskArray.getDisplayString());
                keyVal.setValue(keyValuePair.getValue());
                keyValue.add(keyVal);
            }
        }
