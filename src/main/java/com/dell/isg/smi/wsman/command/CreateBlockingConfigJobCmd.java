...
public class CreateBlockingConfigJobCmd extends WSManBaseCommand {

    private static final Logger logger = LoggerFactory.getLogger(CreateBlockingConfigJobCmd.class);
    ...
    public CreateBlockingConfigJobCmd(String ipAddr, String userName, String passwd, boolean pollJobStatus) {
        super(ipAddr, userName, passwd);
        if (logger.isTraceEnabled()) {
            logger.trace(String.format("Entering constructor: CreateBlockingConfigJobCmd(String ipAddr - %s, String userName - %s, String passwd - %s, boolean pollJobStatus - %s)", ipAddr, userName, "####", Boolean.toString(pollJobStatus)));
        }
        ...
        logger.trace("Exiting constructor: CreateBlockingConfigJobCmd()");
    }
    ...
    public ConfigJobDetail execute() throws Exception {
        ...
        logger.info("Entering createBlockingConfigJob.execute() ");
    ...
    private List<String> createConfigJob() throws Exception {
        ...
        int retries = 5;
        /*
         * for (int i = 0; i < retries; i++) { attempts++;
         * 
         * Addressing response = session.sendInvokeRequest();
         * 
         * // Get the jobID out of the response. XPathFactory xPathFactory = XPathFactory.newInstance(); XPath xpath = xPathFactory.newXPath(); SOAPBody soapBody =
         * response.getBody();
         * 
         * String returnValue = xpath.evaluate("*[local-name()='CreateTargetedConfigJob_OUTPUT']/*[local-name()='ReturnValue']",soapBody);
         * 
         * // Only if it is a Successful return - get the JOB-Id and return that // If the job is created - the return code would be 4096. // It would be 0 if there is no job
         * created if (StringUtils.equalsIgnoreCase(returnValue,WSCommandRNDConstant.SUCCESSFULL_CONFIG_JOB_RETURN) ||
         * (StringUtils.equalsIgnoreCase(returnValue,WSCommandRNDConstant.SUCCESSFULL_UPDATE_JOB_RETURN))) {
         * 
         * returnList.add(getJobID(soapBody, xpath)); returnList.add(getResourceURI(soapBody, xpath));
         * 
         * return returnList;
         * 
         * } else { messageFromConfigJob = xpath.evaluate("*[local-name()='CreateTargetedConfigJob_OUTPUT']/*[local-name()='Message']",soapBody);
         * logger.info("Received the message from createJob :: " + messageFromConfigJob);
         * 
         * String LCErrorCode = session.getLCMessageID(); if( LCErrorCode != null && !LCErrorCode.isEmpty() ){
         * 
         * if(WSManUtilities.noPendingDataErrorCodes.contains(LCErrorCode)){ //No pending data present to create a Configuration job return returnList; }
         * if(WSManUtilities.systemServicesInUseErrorCodes.contains(LCErrorCode)){ //System services is currently in use this.waitForReadyStatus(CommonConstants.TEN_MIN); // block
         * until system services are available or timeout logger.info("Retrying CreateBlockingConfigJob command..."); continue; } if
         * (WSManUtilities.existingJobCompleteOrCancelErrorCodes.contains(LCErrorCode)){
         * logger.error("Existing job must be completed or cancelled before creating new config job. No retry will be attempted. "); break; } }
         * 
         * // if we have a job ID of a failed job, delete it. String jobId = getJobID(soapBody, xpath); if(jobId != null && !jobId.isEmpty()){ logger.info("Deleting failed job " +
         * jobId + " (with return code: " + returnValue + ")"); deleteJob(jobId); }
         * 
         * //logger.error("Retrying CreateConfigJob command..."); Thread.sleep(CommonConstants.THIRTY_SEC); } }
         * 
         * //logger.error("Completed " + retries + " attempts: Error while creating the reboot job :: "+ messageFromConfigJob); //RuntimeCoreException ex =
         * ExceptionUtilities.getSpectreRuntimeException(267065,messageFromConfigJob, this.getLCErrorCode(), this.getLCErrorStr()); //throw ex;
         */
        return null;
    }
