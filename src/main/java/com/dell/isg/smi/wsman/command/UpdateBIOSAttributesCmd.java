...
public class UpdateBIOSAttributesCmd extends WSManBaseCommand {
	...
	public UpdateBIOSAttributesCmd(String ipAddr, String userName, String passwd, Map<String, String> attributes,
			boolean isCreateConfigJob) {

		super(ipAddr, userName, passwd);
		if (logger.isTraceEnabled()) {
			logger.trace(String.format(
					"Entering constructor: UpdateBIOSAttributesCmd(String ipAddr - %s, String userName - %s, String passwd - %s, boolean isCreateConfigJob - %s)",
					ipAddr, userName, "####", Boolean.toString(isCreateConfigJob)));
		}
		session = this.getSession();
		this.createConfigJob = isCreateConfigJob;
		this.attributeNames = attributes.keySet().stream().sorted().collect(Collectors.toList());
		...
		for (Map.Entry<String, String> entry : attributes.entrySet()) {
			this.session.addUserParam("AttributeName", entry.getKey());
			this.session.addUserParam("AttributeValue", entry.getValue());
		}
	...
	private void handlePendingConfigurationJob(String messageFromConfigJob) {
		...
		// Configuration job already created, cannot set attribute on specified
		// target until existing job is completed or is cancelled
		if (StringUtils.contains(messageFromConfigJob, "existing job is completed or is cancelled")) {

			DeleteJobCmd deleteCmd = new DeleteJobCmd(this.session.getIpAddress(), this.session.getUser(),
					this.session.getPassword(), "JID_CLEARALL");
	...
	public String execute() throws Exception {
		...
		for (int i = 0; i < MAX_RETRY; i++) {
			...
			if (null != response) {
				...
				if (null != setAttributesOutput) {
					// Only if it is a Successful return , which is 0 - get the
					// Message and set it on the success string
					if (StringUtils.equalsIgnoreCase(setAttributesOutput.getReturnValue().getValue() + "",
						...
						// check if there are more than one message and if
						// atleast one of them is completed successfully.
						if (setAttributesOutput.getMessage() != null && setAttributesOutput.getMessage().size() > 0) {
							for (CimString cimString : setAttributesOutput.getMessage()) {
								...
								// If the command is successful, the string will
								// be "The command was successful"
					...
					// if there is atleast one successfull command - we don't
					// have to worry about the error - saying "System Services
					// currently in use"
					if (StringUtils.isNotBlank(attrSucceededStr) && StringUtils.isNotEmpty(attrSucceededStr)) {
						if (exceptionString.length() > 0) { // Some failures
															// occurred. Log
															// them.
							logger.debug("Not all attribute updates were successfully scheduled: "
						...
						if (exceptionString.length() > 0) {
							...
							throw ExceptionUtilities.getCoreRuntimeException(267033, exceptionString.toString(),
									this.getLCErrorCode(), this.getLCErrorStr());
		...
		if (null != attrSucceededStr) {
			...
			logger.error("Exception while updating the attributes : \n" + exceptionString);
			throw ExceptionUtilities.getCoreRuntimeException(267033, exceptionString.toString(), this.getLCErrorCode(),
					this.getLCErrorStr());
		}
	...
	private void createConfigJob(StringBuilder exceptionString) throws Exception {
		...
		CreateConfigJobCmd cmd = new CreateConfigJobCmd(this.session.getIpAddress(), this.session.getUser(),
				this.session.getPassword(), true, "BIOS.Setup.1-1");
	...
	private boolean checkIfAttributesSet() throws Exception {
		...
		EnumerateBIOSAttributeCmd biosAttrCmd = new EnumerateBIOSAttributeCmd(this.session.getIpAddress(),
				this.session.getUser(), this.session.getPassword(), null);
					...
					if (pendingValueList != null && pendingValueList.size() > 0) {
						...
						if (null != pendingValue) {
							...
							if (StringUtils.isNotBlank(pendingValueStr) || StringUtils.isNotEmpty(pendingValueStr)) {
								logger.error("Attribute not set for: " + attributeName.getValue() + " to value: "
										+ pendingValueStr);
								bReturn = false;
							}
						}
