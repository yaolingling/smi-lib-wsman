...
public class SystemEraseCmd extends WSManBaseCommand {

    private static final Logger logger = LoggerFactory.getLogger(SystemEraseCmd.class);
    ...
    public XmlConfig execute() throws Exception {
        ...
        logger.info("Entering execute(), SYSYTEM_EARSE for server {} ", session.getIpAddress());
        String content = XMLTool.convertAddressingToString(session.sendInvokeRequest());
        String jobId = getUpdateJobID(content);
        logger.trace("Exiting function: SYSYTEM_EARSE execute()");
